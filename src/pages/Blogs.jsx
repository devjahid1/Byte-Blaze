...
const Blogs = () => {
    ...
    const {title, comments_count, reading_time_minutes, public_reactions_count, readable_publish_date} = blog;
...
<div className="max-w-3xl px-6 py-16 mx-auto space-y-12">
	<article className="space-y-8 dark:text-gray-900">
		<div className="space-y-6">
			...
			<div className="flex flex-col items-start justify-between w-full md:flex-row md:items-center dark:text-gray-600">
                <p className="text-sm">{reading_time_minutes} min read • {new Date(readable_publish_date).toLocaleDateString()}</p>
	...
	{/* Book Mark Button */}
	<div onClick={()=> handleBookmark(blog)} className="bg-primary p-3 ml-5 rounded-full hover:bg-opacity-30 bg-opacity-20 cursor-pointer hover:scale-105 overflow-hidden"><MdBookmarkAdd size={20} className="text-secondary"/></div>
</div>
		</div>
		<Outlet/>
